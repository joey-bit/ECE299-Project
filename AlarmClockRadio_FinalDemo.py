...
fm_radio = Radio( 101.9, 1, False ) #Initialize radio
mute = False
...
def switch_mode_global():
    global mode_global, encoderIncrement, encoderDecrement, encoderATrigger, encoderBTrigger
    if encoderATrigger == True or encoderBTrigger == True:
        encoderIncrement = False
        encoderDecrement = False
        encoderATrigger = False
        encoderBTrigger = False
...
def set_12_format(hr_obj, which):
    global time_hr, time_min, alarm_hr, alarm_min, clock_time_12, alarm_time_12, ds3231
    time_hr_12 = hr_obj
    if time_hr_12 > 12:
        am_pm = " PM"
        time_hr_12 -=12
        ...
        am_pm = " AM"
    if which == "clock": 
        clock_time_12 = str(('{:02d}'.format(time_hr_12) + ":" + '{:02d}'.format(ds3231.getMinutes()) + am_pm))
    if which == "alarm":
...
def set_time_universal(time_string_obj, hr_obj, min_obj, top_text):
    global toggled_up, toggled_down, toggled_mode, clock_time, alarm_time, time_hr, time_min, alarm_hr, alarm_min, timeset
    cursor = "hour"
    display.clear_buffers()
...
def count_seconds(num_seconds):
    global time_counted, ds3231, seconds_counted, display, toggled_mode, mode_global
    ...
    while (seconds_counted < num_seconds):
        ...
        if ds3231.getSeconds() != initial_secs:
            ...
            initial_secs = ds3231.getSeconds()
        display.clear_buffers()
        display.draw_text( 6, 0, "Snooze Countdown", bally )
        countdown = str(num_seconds-seconds_counted)+" sec"
        display.draw_text( 54, 22, countdown, rototron )
        display.present()
    time_counted = True
    seconds_counted = 0
...
while True:
    ...
    #if not timeset:#changed 3 lines under (back tab)
    ...
    if mode_global == "clock":
        ...
        if (toggled_snooze):
            ...
            timeset = True
            toggled_snooze = 0
        clock_time = ('{:02d}'.format(ds3231.getHour()) + ":" + '{:02d}'.format(ds3231.getMinutes()) + ":" '{:02d}'.format(ds3231.getSeconds()))
    ...
    if mode_global == "ringing alarm":
        ...
        for note in alarm_theme:
            ...
            if (toggled_mode):
                mode_global = "clock"
                toggled_mode = 0
                fm_radio.SetMute( 0 )# Automatically unmute radio after turning off alarm so you can start listening as soon as you wake up
                fm_radio.ProgramRadio()
                break
            if (toggled_snooze):
                mode_global = "snooze"
    ...
    if mode_global == "snooze":
        fm_radio.SetMute( 1 )
        fm_radio.ProgramRadio()
        count_seconds(10)#Set your snooze countdown in seconds here
        mode_global = "ringing alarm"
